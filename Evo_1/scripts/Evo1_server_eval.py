# Evo1_server_eval.py
# Duplicate of Evo1_server.py with two changes:
#   1. __main__: accepts --ckpt_dir, --port, --arm_key, --dataset_key via argparse
#   2. load_model_and_normalizer: loads checkpoint.pt / model_state_dict
#      (original targeted DeepSpeed mp_rank_00_model_states.pt / "module")
...
def load_model_and_normalizer(ckpt_dir):
    ...
    # NOTE: loads checkpoint.pt / model_state_dict (not DeepSpeed mp_rank_00_model_states.pt / "module")
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Evo1 WebSocket inference server")
    parser.add_argument("--ckpt_dir", required=True, help="Path to checkpoint directory")
    parser.add_argument("--port", type=int, default=9000, help="WebSocket port to listen on")
    parser.add_argument("--arm_key", default="metaworld_sawyer", help="Arm key in norm_stats.json")
    parser.add_argument("--dataset_key", default="Evo1_MetaWorld", help="Dataset key in norm_stats.json")
