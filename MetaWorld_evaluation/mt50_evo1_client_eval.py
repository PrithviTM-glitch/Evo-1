# mt50_evo1_client_eval.py
# Duplicate of mt50_evo1_client_prompt.py with one change:
#   __main__: accepts --port and --out_dir via argparse, overrides SERVER_URL and
#   output dirs (LOG_DIR, LOG_PATH, VIDEO_SAVE_DIR, INSPECT_DIR) before running.
...
if __name__ == "__main__":
    import argparse
    _p = argparse.ArgumentParser(description="MT50 MetaWorld evaluator")
    _p.add_argument("--port", type=int, default=9000, help="Server WebSocket port (default: 9000)")
    _p.add_argument("--out_dir", default=".", help="Output directory for logs/videos/frames (default: cwd)")
    _args = _p.parse_args()

    SERVER_URL = f"ws://127.0.0.1:{_args.port}"
    if _args.out_dir != ".":
        LOG_DIR = os.path.join(_args.out_dir, "logs")
        os.makedirs(LOG_DIR, exist_ok=True)
        LOG_PATH = make_log_path("mt50")
        VIDEO_SAVE_DIR = os.path.join(_args.out_dir, "episode_videos")
        INSPECT_DIR = os.path.join(_args.out_dir, "inspect_frames")
